...
Author:      blogsdna | Updated by John Andrew Jose
...
function ald_crp() {
	...
	if($searches){
		...
		//--------------Code to Output WP-Thumbie in Verticle Format---------
				if($crp_settings[related_post_style] == '2') { 
					$attachments = get_posts('post_parent='.$search->ID.'&post_type=attachment&post_mime_type=image');
					if ( $attachments ) {
						foreach ( $attachments as $attachment ) {
							$thumb = wp_get_attachment_image($attachment->ID);
						}
					}
					
					$RelatedPostid = $search->ID;
					$post_id = get_post($RelatedPostid, ARRAY_A);
					$theExcerpt = $post_id['post_excerpt'];
					
				$output .='<li id="wp_thumbie_li">
							<div id="wp_thumbie_image"><a href="'.get_permalink($search->ID).'" target="_top">'.$thumb.'</a></div>
							<div id="wp_thumbie_title"><a href="'.get_permalink($search->ID).'" target="_top">'.$title.'</a></div>
							<div id="description">'.$theExcerpt.'</div>
							<div class="clear"></div>
		...
		//--------------Code to Output WP-Thumbie in Raw Format---------
				if($crp_settings[related_post_style] == '3') { 
					...
					$output .= '<li id="wp_thumbie_li"><div id="wp_thumbie_image"><a href="'.get_permalink($search->ID).'" rel="bookmark" target="_top">'.$thumb.'</div><div id="wp_thumbie_title">'.$title.'</div></a><div id="description">'.$post_text.'</div></li>'; }
				$search_counter++; 
			}
